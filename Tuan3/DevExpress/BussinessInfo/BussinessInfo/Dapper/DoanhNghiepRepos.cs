...
{
    public class DoanhNghiepRepos : BaseAPI
    {
        private string tableName = "DoanhNghiep", propName, propValue, propUpdate;
        ...
        public DoanhNghiep Find(string mst,List<DoanhNghiep> lstDoanhNghiep)
        {
            try
            {
                ...
                var node = lst.Where(x => !string.IsNullOrEmpty(x.MaSoThue) && x.MaSoThue == mst);
        ...
        public DoanhNghiep Find(string mst)
        {
            ...
            {
                string query = "SELECT Id FROM " + tableName + " WHERE MaSoThue = '" + mst + "'";
                return this._db.Query<DoanhNghiep>(query).SingleOrDefault();
            }
            else
            {
        ...
        public DoanhNghiep Update(DoanhNghiep DoanhNghiep)
        {
            if (this._db.State == ConnectionState.Closed)
                _db.Open();
            var sqlQuery = "UPDATE " + tableName + " SET " + propUpdate + " WHERE MaSoThue = @MaSoThue";
